...
他にも、配列を柔軟に作成できる`Array.from()`があります。引数に配列風のオブジェクト（`length`プロパティなどを持つもの）を入れると、配列に変換されます。

```js
// stringを入れると1文字ずつ分割される
// 実はstringは配列風のオブジェクトの1つだから
...
最近だと、第1引数にPromiseが含まれる配列を入れられる`Array.fromAsync()`というものもあります。
...
`arr.shift()`と`arr.pop()`は、元の配列から先頭・末尾の要素がなくなってしまう点に注意が必要です！

:::message
メソッドの実行による自身の配列の変更は、その配列が`const`で宣言したものであっても避けられません。これは`arr.shift()`と`arr.pop()`だけでなく、この後に出てくるメソッドも同様です。
:::
...
`arr.sort()`が定番ですが、**`arr`自身の要素を並び替える点**に注意が必要です。返ってくる配列は、`arr`と同じものです。

元の配列を変えたくないニーズに応える形で、最近は`arr.toSorted()`が使えるようになりました！これは、`arr.sort()`と同じ使い方ですが、元の配列を変えず、並び替え後の新しい配列を返します。

どちらも、引数には並び替え条件を示す関数を設定できます。これを使えばオブジェクトの並び替えもできて便利です。具体例は上にある通りですが、次のような関数にする必要があります。
